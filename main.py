...
class ZombiBoss(pygame.sprite.Sprite):
    def __init__(self, sheet_b, sheet_z, sheet_p, group, tp):
        super().__init__(group)
        self.frames = []
...
def load_level():
    ...
    for elem in ZOMBIE_COORDS[current_level]:
        if current_level == 'level3':
            z = ZombiBoss(load_image("boss_bok.png"), load_image('boss_zad.png'), load_image('boss_pered.png'), boss, elem)
        else:
